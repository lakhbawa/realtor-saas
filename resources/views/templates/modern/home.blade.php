...
<section class="py-12 bg-white border-b">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
                <div class="text-4xl md:text-5xl font-bold text-primary mb-2">{{ $site?->stat_properties_sold ?? ($featuredProperties->count() > 0 ? $featuredProperties->count() * 10 : 50) }}+</div>
                <div class="text-gray-600 font-medium">{{ $site?->stat_properties_sold_label ?? 'Properties Sold' }}</div>
            </div>
            <div>
                <div class="text-4xl md:text-5xl font-bold text-primary mb-2">${{ $site?->stat_sales_volume ?? ($featuredProperties->count() > 0 ? number_format($featuredProperties->sum('price') / 1000000, 0) : 25) }}M+</div>
                <div class="text-gray-600 font-medium">{{ $site?->stat_sales_volume_label ?? 'Sales Volume' }}</div>
            </div>
            <div>
                <div class="text-4xl md:text-5xl font-bold text-primary mb-2">{{ $site?->stat_happy_clients ?? $site?->years_experience ?? 10 }}+</div>
                <div class="text-gray-600 font-medium">{{ $site?->stat_happy_clients_label ?? 'Happy Clients' }}</div>
            </div>
            <div>
                <div class="text-4xl md:text-5xl font-bold text-primary mb-2">{{ number_format($site?->stat_average_rating ?? 5.0, 1) }}</div>
                <div class="text-gray-600 font-medium flex items-center justify-center">
                    <svg class="w-5 h-5 text-yellow-400 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    {{ $site?->stat_average_rating_label ?? 'Star Rating' }}
                </div>
            </div>
        </div>
    </div>
